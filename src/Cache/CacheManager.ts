...
import { mergeDefault, chunk } from "@repeaterts/utilities";
...
export class CacheManager {
  ...
  constructor(directory?: string) {
    if (directory && directory.length === 0)
      directory = dirname(require.main?.filename as string);
    this.baseDirectory =
      resolve(
        directory ?? dirname(require.main?.filename as string),
        "cache"
  ...
  public async getAll(entries: string[]): Promise<unknown[]> {
    if (!Array.isArray(entries) || !entries.length)
      entries = await this.getKeys();
    if (entries.length < 5000) {
      return (await Promise.all(entries.map(this.get.bind(this)))).filter(
        (value) => value
      ) as unknown[];
    }
    ...
    for (const part of parts)
      output.push(...(await Promise.all(part.map(this.get.bind(this)))));
  ...
  public async create(
    ...
    await fs.outputJSONAtomic(resolve(this.baseDirectory, `${id}.json`), {
      id,
      ...data,
    });
  }
  ...
  public async update(id: string, data: unknown): Promise<void> {
    const existent = (await this.get(id)) as Record<
      PropertyKey,
      unknown
    > | null;
    await fs.outputJSONAtomic(
      resolve(this.baseDirectory, `${id}.json`),
      mergeDefault(existent ?? { id }, data as Record<string, unknown>)
    );
  }
