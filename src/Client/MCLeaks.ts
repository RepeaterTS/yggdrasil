...
import utils from '../Util/Util'
     ...
     */
export async function auth(options: { agent?: string, token: string }, agent?: Agent | undefined) {
    return await utils.call(Constants.MCLeaks.DefaultHost, 'redeem', { token: options.token }, agent)
}
     ...
     * @param {Agent} [agent]  User-Agent
     */
export async function join(options: { session: string, mcname: string, serverhash: string, server: string }, agent: Agent | undefined) {
    return await utils.call(Constants.MCLeaks.DefaultHost, 'joinserver', {
