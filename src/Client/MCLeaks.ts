...
const MCLeaks = {
     ...
     */
    auth: async function (options: { agent?: string, token: string }, agent?: Agent | undefined) {
        return await utils.call(Constants.MCLeaks.DefaultHost, 'redeem', {
            token: options.token
        }, agent)
    },
     ...
     * @param {Agent} [agent]  User-Agent
     */
    join: async function (options: { session: string, mcname: string, serverhash: string, server: string }, agent: Agent | undefined) {
